...
export default function TourDetailPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await submitLead({
                ...
                emoji: "🌴",
                fields: { Тур: tour.title, "Количество человек": people, "Желаемая дата": formatDate(date) },
            });
    ...
    return (
        <Layout>
            ...
            {fullscreen && (
                <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-3 sm:p-6" role="dialog" aria-modal="true" aria-label={`Фото ${slide + 1} из ${images.length}`} onClick={() => setFullscreen(false)}>
            ...
            <div className="animate-fade-in-up">
                ...
                <Card padding="md" className="mb-5">
                    ...
                    {tour.facts && <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-5">{tour.facts.map(([label, value]) => <div key={label} className="rounded-xl bg-surface-muted border border-border px-3 py-2.5"><div className="text-xs text-ink-muted">{label}</div><div className="text-sm font-semibold text-ink mt-0.5">{value}</div></div>)}</div>}
                    <div className="mt-5">
                        <h3 className="font-semibold text-ink mb-3">Что вас ждёт</h3>
                        <ul className="space-y-2.5">{tour.highlights.map((item) => <li key={item} className="flex gap-2.5 text-sm text-ink-muted"><span className="text-brand-600">✓</span><span>{item}</span></li>)}</ul>
                    </div>
                    {tour.tips && <div className="mt-5 rounded-xl bg-brand-50 border border-brand-100 p-3.5"><h3 className="font-semibold text-ink mb-2">Полезно знать</h3><ul className="space-y-2">{tour.tips.map((tip) => <li key={tip} className="text-sm text-ink-muted leading-relaxed">• {tip}</li>)}</ul></div>}
                </Card>
                ...
                {completed && <Card className="animate-fade-in-up">
